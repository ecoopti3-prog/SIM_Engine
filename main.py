...
def main():
    ...
    parser.add_argument("--auto",     action="store_true", help="Simulate only new unsimulated Diamonds (for cron)")
    ...
    if args.auto:
        # ── Autonomous mode: simulate only new unsimulated Diamonds ──────────
        from db.reader import load_unsimulated_diamonds, save_sim_result
        from core.dispatcher import dispatch
        from reports.generator import generate_report

        pending = load_unsimulated_diamonds(limit=args.limit)
        if not pending:
            logger.info("[Auto] No new Diamond ideas to simulate — nothing to do")
            sys.exit(0)

        logger.info(f"[Auto] Found {len(pending)} unsimulated Diamond(s) — starting simulation")
        for idea in pending:
            logger.info(f"[Auto] Simulating: {idea['id'][:8]} — {idea.get('title','')[:60]}")
            try:
                result = dispatch(idea)
                result.report_path = generate_report(result)
                save_sim_result(result, idea["id"])
                logger.info(f"[Auto] → {result.overall_status.upper()} score={result.sim_score}/10 saved to Supabase")
            except Exception as e:
                logger.error(f"[Auto] Failed for {idea['id'][:8]}: {e}")
                import traceback; logger.error(traceback.format_exc())

        logger.info(f"[Auto] Complete — {len(pending)} ideas simulated")
        sys.exit(0)
